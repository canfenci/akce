...
export function ExpensesScreen({ openQuick, openFormSignal: _openFormSignal, onFormSignalConsumed: _onFormSignalConsumed }: { openQuick: () => void; openFormSignal?: string | null; onFormSignalConsumed?: () => void }) {
  ...
  return <div className="screen"><PageHeader eyebrow="YAŞAM KASASI" title="Harcamalar" description="Her çıkışı gör, davranışını fark et." />
    ...
    <section className="data-card"><div className="data-card__header"><b>İşlemler</b><span>{visible.length} kayıt</span></div>{visible.map(item => <div className="data-row" key={item.id}><span className="transaction__icon"><Icon name={item.paymentMethod === 'kart' ? 'card' : 'receipt'} /></span><span className="data-row__main"><b>{item.note || item.category || 'Kategorisiz'}</b><small>{new Intl.DateTimeFormat('tr-TR', { day: 'numeric', month: 'long' }).format(new Date(item.date))} · {item.paymentMethod}</small></span><span className={`tag tag--${item.type === 'plansız' ? 'warn' : 'neutral'}`}>{titleCase(item.type)}</span><strong>-{formatCurrency(item.amount)}</strong><button className="delete-button" onClick={() => dispatch({ type: 'REMOVE_EXPENSE', id: item.id })} aria-label="Harcamayı sil"><Icon name="trash" /></button></div>)}</section>
  </div>;
}
...
export function BudgetScreen({ initialTab = 'Gelir', openFormSignal, onFormSignalConsumed }: { initialTab?: BudgetTab; openFormSignal?: string | null; onFormSignalConsumed?: () => void }) {
  ...
  // AKÇE-049: Accordion state for category details
  ...
  const openAddIncome = () => {
    setIncomeName(''); setIncomeAmount(''); setIncomeCategory('salary'); setIncomeRecurring(true); setIncomeError('');
  ...
  const saveIncome = () => {
    ...
    if (incomeForm.mode === 'edit' && incomeForm.currentIncome) {
      dispatch({ type: 'UPDATE_INCOME', payload: { ...incomeForm.currentIncome, name: incomeName.trim(), amount, category: incomeCategory, recurring: incomeRecurring, updatedAt: now } });
    } else {
      dispatch({ type: 'ADD_INCOME', payload: { id: crypto.randomUUID(), name: incomeName.trim(), amount, date: `${state.selectedMonthKey}-01`, recurring: incomeRecurring, active: true, category: incomeCategory, monthKey: state.selectedMonthKey, createdAt: now, updatedAt: now, userId: 'local-user' } });
    }
