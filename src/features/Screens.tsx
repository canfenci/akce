...
export function BudgetScreen({ initialTab = 'Gelir', openFormSignal, onFormSignalConsumed }: { initialTab?: BudgetTab; openFormSignal?: string | null; onFormSignalConsumed?: () => void }) {
  ...
  <div className="screen">
    ...
    {tab === 'Birikim' && <section className="data-card"><div className="data-card__header"><b>Yatırım planları</b><strong>{formatCurrency(summary.totalFixedInvestment)}</strong></div>{state.investments.filter(item => item.monthKey === state.selectedMonthKey).map(item => <div className="data-row" key={item.id}><span className="data-row__main"><b>{item.name}</b><small>{item.group}</small></span><strong>{formatCurrency(item.plannedAmount)}</strong></div>)}</section>}
    {tab === 'Özet' && <section className="data-card"><div className="data-card__header"><b>Aylık özet</b></div><div className="metric-grid"><div className="metric metric--lead"><span>Gelir Toplamı</span><strong>{formatCurrency(summary.totalIncome)}</strong></div><Metric label="Gider Toplamı" value={formatCurrency(summary.totalAutomaticExpenses + summary.totalVariableExpenses)} /><Metric label="Birikim Toplamı" value={formatCurrency(summary.totalFixedInvestment)} /><Metric label="Serbest Bütçe" value={formatCurrency(summary.remainingBudget)} /></div></section>}
