...
export function QuickExpenseSheet({ open, onClose, initialCategory = 'Market', initialType = 'zorunlu', initialPaymentMethod = 'kart', onSave }: {
  ...
  const save = () => {
    ...
    const savedCategory = hasCategories ? validCategory : '';
    dispatch({ type: 'ADD_EXPENSE', payload: { id: crypto.randomUUID(), amount: numericAmount, category: savedCategory, type: expenseType, paymentMethod, note: note.trim() || undefined, date, monthKey: state.selectedMonthKey, createdAt: now.getTime(), updatedAt: now.getTime(), userId: 'local-user' } });
    onSave?.(savedCategory, expenseType, paymentMethod);
  ...
  return <div className="sheet-layer" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="sheet" role="dialog" aria-modal="true" aria-labelledby="expense-title" ref={sheetRef} style={{ maxHeight: sheetMaxHeight }}>
      ...
      {hasCategories
        ...
        : <div className="field-info">Henüz kategori yok. Harcamayı kategorisiz kaydedebilirsin.</div>}
